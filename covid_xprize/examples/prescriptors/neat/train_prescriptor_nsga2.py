...
from covid_xprize.examples.prescriptors.neat.nsga2 import NSGA2Fitness, NSGA2Reproduction
...
case_coeff = float(args.coeff)
print(f'case coeff: {case_coeff}, stringency coeff: {1 - case_coeff}')

# Path where this script lives
ROOT_DIR = Path(os.path.dirname(os.path.abspath(__file__)))

# Config file for running NEAT (expected to reside in same dir as this script)
NEAT_CONFIG_FILE = ROOT_DIR / 'config-nsga2-prescriptor'
...
def eval(genome, geo_costs, config):
    genome_id, genome = genome
    genome.fitness = NSGA2Fitness(0,0)
    ...
    # genome.fitness = -((new_cases*case_coeff) + ((1 - case_coeff) * stringency))
    
    obj = np.array([new_cases, stringency])
    fit_vec = ((obj - utopia) / (nadir - utopia)) * w
    genome.fitness.add(-fit_vec[0], -fit_vec[1])
    
    print('Evaluated Genome', genome_id)
    print('New cases:', new_cases)
    print('Stringency:', stringency)
    print('Fitness:', -fit_vec)
...
def eval_genomes(genomes, config):
    ...
    params = [[genome, geo_costs, config] for genome in genomes]
    cpus = np.min([64, mp.cpu_count(), len(params)])
    ...
    with mp.Pool(cpus) as p:
        r_genomes = p.starmap(eval, params)
        p.close()
        p.join()
...
# Load configuration.
config = neat.Config(neat.DefaultGenome, NSGA2Reproduction,
...
p.add_reporter(neat.Checkpointer(generation_interval=1,
                                 ...
                                 filename_prefix=f'nsga2-neat-checkpoint-'))
